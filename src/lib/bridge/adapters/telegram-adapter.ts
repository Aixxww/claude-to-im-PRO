...
export class TelegramAdapter extends BaseChannelAdapter {
   ...
   * Useful for progress updates ("working..." → result).
   */
  async editMessage(chatId: string, messageId: string, text: string, parseMode?: 'HTML' | 'Markdown'): Promise<TelegramSendResult> {
    const token = this.botToken;
    if (!token) return { ok: false, error: 'No bot token configured' };
  ...
  /**
   * Send a progress update message, then edit the result when done.
   * Returns the working message ID for later editing.
   */
  async sendProgressUpdate(chatId: string, workingText: string): Promise<{ ok: boolean; messageId?: string; error?: string }> {
    const address = { channelType: 'telegram' as const, chatId };
    return this.send({ address, text: workingText, parseMode: 'plain' });
  }

  /**
   * Add emoji reaction to a message.
   * Only supports Telegram's fixed emoji whitelist.
   */
  async addReaction(chatId: string, messageId: string, emoji: string): Promise<{ ok: boolean; error?: string }> {
    const token = this.botToken;
    if (!token) return { ok: false, error: 'No bot token configured' };

    const result = await callTelegramApi(token, 'setMessageReaction', {
      chat_id: chatId,
      message_id: messageId,
